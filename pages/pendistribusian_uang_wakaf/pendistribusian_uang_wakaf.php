<?php
    ...
    <select class='form-control' required name='id_pengembangan__uang_wakaf'>"?><?php echo"
        <option value=''>-- Pilih WAKIF --</option>"?><?php
        
        // mengamil data dari tabel wakif
$wakifQuery = mysqli_query($conn, "
SELECT 
    p.id_pengembangan__uang_wakaf,
    w.nama_wakif
FROM 
    pengembangan_uang_wakaf p
INNER JOIN uang_wakaf u ON p.id_uang_wakaf = u.id_uang_wakaf
INNER JOIN wakif w ON u.id_wakif = w.id_wakif
WHERE NOT EXISTS (
    SELECT 1 
    FROM pendistribusian_uang_wakaf d 
    WHERE d.id_pengembangan__uang_wakaf = p.id_pengembangan__uang_wakaf
)
") or die(mysqli_error($conn));
    ...
    <select class='form-control' required name='id_penerima_manfaat_wakaf'required >"?><?php echo"
...
$wakifQuery = mysqli_query($conn, "SELECT id_penerima_manfaat_wakaf, nama_penerima FROM penerima_uang_wakaf");
        ...
        ?><?php echo"
					...
					<input type='text' required class='form-control' placeholder='Besar Manfaat Wakafditerima' name='besar_manfaat_wakafditerima' value='"?><?php echo isset($c['besar_manfaat_wakafditerima']) ? $c['besar_manfaat_wakafditerima'] : '';?><?php echo"'"?> <?php echo isset($c['besar_manfaat_wakafditerima']) ? ' ' : ' ';?><?php echo" >
					...
					<input type='text' required class='form-control' placeholder='Tujuan Pengunaan Wakaf Uang' name='tujuan_pengunaan_wakaf_uang' value='"?><?php echo isset($c['tujuan_pengunaan_wakaf_uang']) ? $c['tujuan_pengunaan_wakaf_uang'] : '';?><?php echo"'"?> <?php echo isset($c['tujuan_pengunaan_wakaf_uang']) ? ' ' : ' ';?><?php echo" >
										</div>
					<div class='form-group'><label >PETUGAS PELAKSANA</label>
					<input type='text' required class='form-control' placeholder='Petugas Pelaksana' name='petugas_pelaksana' value='"?><?php echo isset($c['petugas_pelaksana']) ? $c['petugas_pelaksana'] : '';?><?php echo"'"?> <?php echo isset($c['petugas_pelaksana']) ? ' ' : ' ';?><?php echo" >		
          <input type='hidden' id='petugas_pelaksanaraw' name='petugas_pelaksanaraw' />
          </div>
					...
					while ($m = mysqli_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td><a href='index.php?page=pendistribusian_uang_wakaf&act=form&id=$m[id_pendistribusian_uangwakaf]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a>
							    ...
							    <a href='javascript:void(0)' onclick='showDetaildistribusi({$m['id_pendistribusian_uangwakaf']})'class='w3-text-green'><i class='fa fa-eye w3-large'></i></a>
						    	<a href='javascript:void(0)' onclick='showprinttandaterimawakaf({$m['id_pendistribusian_uangwakaf']})'class='w3-text-green'><i class='fa fa-print w3-large'></i></a>
...
<script>    
function showprinttandaterimawakaf(id) {
    $('#detailPopuptt').modal('show');
  $.get('pages/pendistribusian_uang_wakaf/prin_tt_manfaatwakaf.php?action=view&id=' + id, html => {
    $('#detailContentt').html(html);
  }).fail(() => {
    $('#detailContentt').html('<div class="alert alert-danger">Gagal memuat data</div>');
  });
}
  // Print eksternal
...
function showDetaildistribusi(id) {
    $('#detailPopupdist').modal('show');
    $.ajax({
        url: 'pages/pendistribusian_uang_wakaf/popupdistribusi.php?action=view&id=' + id,
        type: 'GET',
        success: function(response) {
            $('#detailContentdist').html(response);
        },
        error: function() {
            $('#detailContentdist').html('<div class="alert alert-danger">Gagal memuat data</div>');
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const idNazirInput = document.getElementById('id_nazir');

    select.addEventListener('change', function () {
        const idNazir = this.options[this.selectedIndex].getAttribute('data-id_nazir');
        idNazirInput.value = idNazir || '';
    });
});
</script>
